...
//  0-10=performance; 11-99=balanced; 100+ = paranoid
...
mod tests {
    ...
    #[test]
    fn rule_942340_false() {
        let rules = rules_inbound();
        let rule = &rules[78];
        let data: Bytes =  Bytes::from_static(b"{\"key1\":\"value1\"}");
        let cap = search(rule, data);
        println!("Cap: {:?}", cap);
